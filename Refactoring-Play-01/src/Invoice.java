...
public class Invoice {
	...
	public String statement(){
		...
		for(Performance performance: performances){
			...
			case TRAGEDY:
				thisAmount += 4_000_000;
				if(performance.getAudienceSize()>30)
					thisAmount += 100_000*(performance.getAudienceSize()-30);
				break;
			case COMEDY:
				thisAmount += 3_000_000;
				if(performance.getAudienceSize()>20)
					thisAmount += 1_000_000+50_000*(performance.getAudienceSize()-20);
				thisAmount += 30_000*performance.getAudienceSize();
				break;
			}
			creditPoints += Math.max(performance.getAudienceSize()-30, 0);
			if(performance.getPlay().getPlayType()==PlayType.COMEDY) {
				creditPoints += performance.getAudienceSize()/5;
			}
